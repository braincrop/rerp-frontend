import { axiosLocal, } from "../axiosConfig"
...
export const WsUpdateproducts = async (data) => {
  const { id, updatedData } = data
   console.log("products-data",data)
    const response = await axiosLocal.post(`ws/${id}/update-specific-product`,{
      ...updatedData
    })
    return response.data
...
export const WsBranchCheckout = async (data) => {
  const { id, updatedData } = data
   console.log("updated-data",data)
    const response = await axiosLocal.post(`ws/${id}/checkout`,{
      ...updatedData
    })
    return response.data
}
